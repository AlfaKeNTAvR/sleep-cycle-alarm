...
41. **A DISMISS never disarms the band; only a SET into the same slot replaces what it is armed with.** The app sent DISMISS SCA-B at 10:04:33Z, every table from 10:19:34Z on showed slot 2 as `enabled:false, title:""`, and the band vibrated at that slot's old 07:04 anyway (11:04Z). Slot 1's intermediate values (07:34, 08:04, 08:11) never rang, because each was overwritten by a later SET into the same slot - so "last write per slot wins" is confirmed, not assumed. The two-title protocol (decision 11) used two slots, so exactly two times could be armed, and exactly two rang.

42. **One title, one slot, all night.** `BAND_ALARM_TITLE_B`, `nextBandAlarmTitle`, `chooseBandAlarmSlotMode`, `BandAlarmSlotMode` and every two-slot branch are deleted, not flagged off. A move is DISMISS then SET under the same title, in one tick. The dismiss-first order is now the safe one: between the two commands the band still holds the OLD time, never nothing. Kept unchanged: the bounded re-sends, orphan adoption, the blind-mode freeze, exact-title matching. The title's VALUE stays `"SCA-A"` so a slot already carrying it from an earlier night is still recognised as ours and reclaimed rather than left armed beside a new one.
...
49. **Deleted the "band alarm mode" status line and its strings.** With one protocol there is no mode to name. The only thing that still varies per tick is whether the table could be read at all, which is now one amber line (`warning_band_alarm_blind`) derived from `lastSyncOk`, plus the `band_alarm_tick` log event.
