...
data class NightState(              // persisted as JSON in filesDir/night_state.json, written atomically (temp file + rename)
    ...
    val lastPlan: AlarmPlan?, val activeBandAlarmTitle: String?,   // "SCA-A" or "SCA-B"
...
Log event types: `night_start`, `tick` (scheduled time vs actual time), `sync` (ok, duration ms, step that failed, cause), `data` (segment count, newest sample time, sleep state), `plan` (mode, band alarm, phone alarm, cycles, the engine's reason), `band_alarm_set`, `band_alarm_dismissed`, `phone_alarm_set`, `phone_alarm_fired`, `error`, `night_end` (with the night summary).

`runNightTick(context, now)`: the one function that does a whole cycle, in this order: load state -> `syncAndReadBandData` -> on failure keep the previous segments and log it -> `computeAlarmPlan` -> if the band alarm minute changed: set the new alarm under the OTHER title, then dismiss the previous title (decision 11) -> (re)arm the phone alarm if it changed -> save state -> schedule the next tick with `nextSyncDelay`. A failed sync never removes an existing alarm. If the band alarm time is already in the past, do not set it; log it; the phone alarm is the safety net. Which protocol a tick uses is read off the band's own alarm table, never a setting (`BandAlarmSlotMode.kt`): with two or more usable slots (free non-smart slots plus the ones we hold) the alternating-title protocol above applies; with exactly one, a move is instead DISMISS our own title then SET it again under the same title, in that order, in one tick, so the band is briefly without an alarm - the only place a dismissal precedes a confirmed replacement. A single-slot SET the next tick's table does not show is re-sent at once, bounded by `MAX_SINGLE_SLOT_RESENDS_PER_NIGHT` per night. Readiness needs one usable slot, not two; with exactly one the setup check adds a non-blocking line about freeing a second.
...
`startNight(context, settings, now)` and `endNight(context, now)`: end dismisses both band alarm titles, cancels phone alarm and ticks, writes `night_end`, clears state, stops the service.
...
- **Setup checklist** (shown until everything is green, reachable later from a gear icon): Gadgetbridge installed; band MAC (text field, prefilled `AA:BB:CC:DD:EE:FF`); export file picked (`ACTION_OPEN_DOCUMENT` + `takePersistableUriPermission`); notifications allowed; full-screen alarms allowed; battery optimisation exemption; a plain list of the Gadgetbridge settings to switch on (from findings.md plus: auto export with a location, Intent API "allow database export" and "broadcast on export"); a "Test connection" button that runs one `syncAndReadBandData` and shows the outcome in plain words.
...
- **Logs**: list of night logs, each shareable through the share sheet (`FileProvider`).
...
JVM unit tests (JUnit 5, no emulator) for everything that is pure: night state JSON round trip, log line formatting, the SQL row to `SleepSegment` mapping (extract the mapping into a pure function so it is testable), the choice of the next band alarm title, UI state derivation from `NightState` + engine output. Android-only glue is not unit tested in v1; it is checked on the phone.
