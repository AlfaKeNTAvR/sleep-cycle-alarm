...
- **A dismissed alarm still goes off** (night 1, 2026-09-18). The row reads `enabled:false, title:""` in the export and the band vibrates at its old time anyway; only a SET into that same slot replaces it. Gadgetbridge source: `DeviceAlarmReceiver` sets `enabled=false` and calls `onSetAlarms` with the whole list, and `AlarmsRequest.addEventAlarm` does send that alarm with `status=false` - but this band firmware honours only the time. The one path that really clears an alarm is `unused` (`if (!alarm.getUnused())` in `addEventAlarm`; an all-unused list serialises to "disable them all"), and `unused` is set only by long-pressing the alarm in Gadgetbridge's own list. The Intent API cannot reach it. So the app can overwrite a band alarm but never remove one.
